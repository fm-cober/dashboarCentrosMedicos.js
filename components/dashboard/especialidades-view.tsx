...
export function EspecialidadesView() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      setError(null)
      try {
        const res = await fetch(`${API_BASE}?view=especialidades`, { cache: "no-store" })
        const json = await res.json()
        if (!json.ok) throw new Error(json.error || "Error al cargar especialidades")
    ...
    <Card>
      ...
      <CardContent>
        {loading ? (
          ...
          <p className="text-sm text-muted-foreground">No hay datos en IMPORT_especialidades.</p>
        ) : (
          <>
